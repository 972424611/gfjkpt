    ...
    $.ajax({
        ...
        success : function (jsonResult) {
            ...
            if(dateType === 'day'){
                ...
                chartTitle = titleDateLeft + '至' + titleDateRight + '功率监测';
            } else if(dateType === 'month'){
                titleDateLeft = getNowChineseDate(date);
                titleDateRight = getNowChineseDate(getNextMonth(date));
                chartTitle = titleDateLeft + '功率监测';
            } else if(dateType === 'quarter'){

            } else if(dateType === 'year'){
                chartTitle = getNowChineseDate(date)  + '功率监测';
            }
...
function iconPreview() {//五个图片的数据加载
    $.ajax({
        type : 'post',
        dataType : 'json',
        url : doMainName + '/load/loadIcon',
        success : function (data) {
            var result = data.data;
            $('#currentLoad').html(result.currentLoad + 'kWh');
            $('#ratio').html(result.ratio + '%');
            $('#percentage').html(result.percentage + '%');
            $('#maxMouthLoad').html(result.maxMouthLoad + 'kWh');
            $('#maxYearLoad').html(result.maxYearLoad + 'kWh');
        },
        error : function (err) {
            console.log('err : ',err);
        }
